...
class HTMLTable(object):
    ...
    def title(self):
        if self.args.title:
            return "<h3>%s</h3>" % self.args.title

    def tHeader(self):
        args = self.args
        headers = re.split(args.fs, args.header, args.max_split)
        _format = '''<th style="padding: 0px 10px;">%s</th>'''
        headers = [_format % i for i in headers]
    ...
    def tBody(self):
        args = self.args
        _format = '''<td style="border: 1px solid #E6EEEE; padding: 0px 5px;">%s</td>'''
        ret = []
        for line in args.lines:
            cols = re.split(args.fs, line, args.max_split)
            cols = [_format % i for i in cols]
            ret.append("<tr>" + "\n".join(cols) + "<tr/>")
...
def getArgs():
    ...
    parser.add_argument('-H', '--host', default='localhost',
                        help="mailer host; Gmail=smtp.gmail.com")
    ...
    parser.add_argument('--title', help="the title for the table body")
    parser.add_argument('--fs', default=r'\s+',
                        help='split columns with; in r"regex" syntax; default r"\s+"')
